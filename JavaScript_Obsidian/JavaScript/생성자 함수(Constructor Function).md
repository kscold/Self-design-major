...
프로토타입(prototype)을 사용하여 프로퍼티와 함수를 추가할 수 있습니다.
...
UserInfo 생성자 함수에는 name 프로퍼티만 존재했는데, prototype을 사용하여 age 프로퍼티를 추가했습니다.
...
new 연산자 없이 생성자 함수를 호출하는 경우 new.target이라는 프로퍼티를 사용하여 new 연산자 사용 여부를 확인할 수 있습니다.

new 연산자 사용 여부를 확인하기 위해 생성자 함수에 new.target 프로퍼티를 추가하고 객체를 생성합니다.
...
![[JavaScript]생성자 함수(Constructor Function) - undefined - 생성자 모드 테스트](https://blog.kakaocdn.net/dn/VUMtZ/btrnJBwuW85/55TrU4WkshJmtxRH1pQa3k/img.png "[JavaScript]생성자 함수(Constructor Function) - undefined - 생성자 모드 테스트")

new 연산자를 사용하지 않고 생성자 함수를 호출하는 경우 new.target은 undefined입니다.
...
인수가 없는 생성자 함수를 호출하는 경우 다음 예제처럼 괄호를 생략할 수 있습니다.
...
객체가 무슨 타입인지 혹은 어떤 생성자 함수로 생성되었는지 확인하고 싶은 경우 instanceof 연산자를 사용할 수 있습니다.

다음 예제는 user라는 객체가 생성자 UserInfo()에 의해 생성되었으므로 true가 반환됩니다.
...
아래 포스팅은 생성자 함수에서 reutnr문에 작성된 값에 따라 다르게 동작하는 것을 자세하게 설명합니다.

출처: [https://developer-talk.tistory.com/281](https://developer-talk.tistory.com/281) [DevStory:티스토리]